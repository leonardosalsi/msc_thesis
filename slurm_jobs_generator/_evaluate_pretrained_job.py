...
def ask_task(parent_folder, verified_model_ids):
    ...
    while True:
        ...
        if success:
            ask_samples(parent_folder, verified_model_ids, verified_task_ids)
            break

def ask_samples(parent_folder, model_ids, task_ids):
    ...
    while True:
        ...
        try:
            ...
            if samples < 1 or samples > 20:
                ...
                _generate_file_for_training(parent_folder, model_ids, task_ids, samples)
                break
        except ValueError:
            print("Invalid input. Select a number between 1 and 20.")

def _generate_file_for_training(parent_folder, model_ids, task_ids, samples):
    shellfiles = []
    ...
    slurm_jobs_folder_name = f"eval-models-from-trained-{joined_model_ids}-{joined_task_ids}-{samples}-lora"
